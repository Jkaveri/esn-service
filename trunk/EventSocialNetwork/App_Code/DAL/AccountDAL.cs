...
public class AccountDAL
{
    ...
    {
        List<AccountInfo> list = context.AccountInfos.Where(a => (a.Account.Email.Contains(keyword) | a.Name.Contains(keyword))
                                                                    & a.Status == 1 & a.AccID != userID).ToList();
        if (list.Count() > 0)
        {
            return list;
        }
    ...
    {
        AccountInfo q = context.AccountInfos.SingleOrDefault(a => a.Account.Email.Equals(email) & a.Account.Password.Equals(password) & a.Status == 1);
        return q;
    }
